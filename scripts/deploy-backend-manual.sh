...
AZURE_OPENAI_MONTHLY_RECAP_DEPLOYMENT="${AZURE_OPENAI_MONTHLY_RECAP_DEPLOYMENT:-}"
MONTHLY_RECAP_REQUIRE_LLM="${MONTHLY_RECAP_REQUIRE_LLM:-}"
AZURE_OPENAI_DOCS_QA_DEPLOYMENT="${AZURE_OPENAI_DOCS_QA_DEPLOYMENT:-}"
...
NOTION_DOCS_DATA_SOURCE_ID="${NOTION_DOCS_DATA_SOURCE_ID:-}"
...
if [ -n "$NOTION_API_TOKEN" ]; then
  SECRET_ARGS+=("notion-api-token=${NOTION_API_TOKEN}")
fi
if [ -n "$SLACK_BOT_TOKEN" ]; then
  SECRET_ARGS+=("slack-bot-token=${SLACK_BOT_TOKEN}")
fi
if [ -n "$SLACK_SIGNING_SECRET" ]; then
  SECRET_ARGS+=("slack-signing-secret=${SLACK_SIGNING_SECRET}")
fi
...
if [ -n "$AZURE_OPENAI_MONTHLY_RECAP_DEPLOYMENT" ]; then
  ENV_ARGS+=("AZURE_OPENAI_MONTHLY_RECAP_DEPLOYMENT=${AZURE_OPENAI_MONTHLY_RECAP_DEPLOYMENT}")
fi
if [ -n "$MONTHLY_RECAP_REQUIRE_LLM" ]; then
  ENV_ARGS+=("MONTHLY_RECAP_REQUIRE_LLM=${MONTHLY_RECAP_REQUIRE_LLM}")
fi
if [ -n "$AZURE_OPENAI_DOCS_QA_DEPLOYMENT" ]; then
  ENV_ARGS+=("AZURE_OPENAI_DOCS_QA_DEPLOYMENT=${AZURE_OPENAI_DOCS_QA_DEPLOYMENT}")
fi
...
if [ -n "$NOTION_API_TOKEN" ]; then
  ENV_ARGS+=("NOTION_API_TOKEN=secretref:notion-api-token")
fi
if [ -n "$NOTION_DOCS_DATA_SOURCE_ID" ]; then
  ENV_ARGS+=("NOTION_DOCS_DATA_SOURCE_ID=${NOTION_DOCS_DATA_SOURCE_ID}")
fi
if [ -n "$NOTION_API_VERSION" ]; then
  ENV_ARGS+=("NOTION_API_VERSION=${NOTION_API_VERSION}")
fi
if [ -n "$SLACK_BOT_TOKEN" ]; then
  ENV_ARGS+=("SLACK_BOT_TOKEN=secretref:slack-bot-token")
fi
if [ -n "$SLACK_SIGNING_SECRET" ]; then
  ENV_ARGS+=("SLACK_SIGNING_SECRET=secretref:slack-signing-secret")
fi
if [ -n "$SLACK_BOT_USER_ID" ]; then
  ENV_ARGS+=("SLACK_BOT_USER_ID=${SLACK_BOT_USER_ID}")
fi
